...
export class ConfigService {
  ...
  private validateInput(envConfig: EnvConfig): EnvConfig {
    ...
    const { error, value: validatedEnvConfig } = Joi.any().validate(envConfig);
    if (error) {
  ...
  get siteUrl(): string {
    return this.envConfig.SITE_URL;
  }
  ...
  getPathComponentUrl(s : any): string {
    if ( Array.isArray(s) ) {      
      s.unshift(this.siteUrl);
      return s.join('/');
    }
    return [this.siteUrl, s].join('/');
  }
}
